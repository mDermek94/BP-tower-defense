...
import argparse
import os
from config import CURRENT_BOARD_FILE, SCREEN_HEIGHT, SCREEN_WIDTH
...
def generate_waypoints_path(source, target, num_waypoints, offset_strength = 2):
    ...
    for i in range(1, num_waypoints + 1):
        ...
        # Normalize
        length = max((perp_x**2 + perp_y**2) ** 0.5, 0.0001) # Avoid division by zero - AI suggestion - ChatGPT
        perp_x /= length
        perp_y /= length
...
def generate_waypoints_global(source, target, num_waypoints = 0):
    ...
    result = sorted( # Generated by AI - ChatGPT
        waypoints,
...
def generate_parameters(difficulty):
    ...
    # Some values generated by AI - ChatGPT
    
    map_D = min(difficulty, 20)
    t = map_D / 20
    
    use_global = not bool(t < 0.5)
    ...
    waypoint_offset_strength = int(1 + (1 - t) * 4) # Formula suggested by AI - ChatGPT
    
    if t < 0.4:
        max_start_end_dist = None
        min_start_end_dist = 10
    else:
        max_start_end_dist = int(20 - t * 12) # Formula suggested by AI - ChatGPT
